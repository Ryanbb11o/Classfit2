...
import { X, ShieldCheck, Fingerprint, Edit3, CheckSquare, Plus, Loader2, Save, Languages, Percent, Image as ImageIcon, Briefcase } from 'lucide-react';
...
const RoleManagementModal: React.FC<RoleManagementModalProps> = ({ user, onClose, onUpdate, language, isManagement, isSelf }) => {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const isTrainer = selectedRoles.includes('trainer') || selectedRoles.includes('trainer_pending');
      const finalName = isTrainer && editSpecialty ? `${editName} (${editSpecialty})` : editName;
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-dark/95 backdrop-blur-xl animate-in fade-in duration-300 text-left">
       <div className="bg-[#1a2332] border border-white/10 rounded-[3rem] p-8 md:p-12 w-full max-w-xl shadow-2xl relative overflow-hidden flex flex-col max-h-[95vh]">
          ...
          <button onClick={onClose} className="absolute top-8 right-8 text-slate-500 hover:text-white bg-white/5 p-2 rounded-full z-10">
            <X size={20} />
          </button>
          
          <div className="mb-10">
             <h2 className="text-3xl font-black uppercase italic text-white tracking-tighter leading-none mb-1">
                {isTrainerMode ? 'COACH MANAGEMENT' : 'IDENTITY MANAGEMENT'}
             </h2>
             <p className="text-slate-500 text-[9px] font-black uppercase tracking-widest italic">
                Update professional profile and financial terms
             </p>
          </div>
          ...
          <div className="overflow-y-auto pr-2 custom-scrollbar space-y-10 pb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                   <label className="text-[9px] font-black uppercase text-slate-500 ml-2">Име</label>
                   <input 
                      value={editName} 
                ...
                <div className="space-y-2">
                   <label className="text-[9px] font-black uppercase text-slate-500 ml-2">Specialty</label>
                   <input 
                      value={editSpecialty} 
            ...
            {isManagement && (
               <div className="p-8 bg-brand/5 rounded-[2.5rem] border border-brand/20 relative group">
                  <div className="flex justify-between items-center mb-6">
                     ...
                     <span className="text-[8px] font-black text-slate-500 uppercase">Current: {user.commissionRate}%</span>
                  </div>
            ...
            {/* Image Link */}
            <div className="space-y-2">
               <label className="text-[9px] font-black uppercase text-slate-500 ml-2">Public Image Link</label>
               <div className="relative">
                  <input 
                     value={editImage} 
            ...
            {/* Authority Delegation - Management Only */}
            {isManagement && (
          ...
          <div className="pt-6">
             ...
             >
                {isSaving ? <Loader2 className="animate-spin" /> : showSuccess ? 'Changes Applied' : <><Save size={18}/> Commit Changes</>}
             </button>
          </div>
       </div>
    </div>
