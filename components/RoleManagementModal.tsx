...
import { X, ShieldCheck, Fingerprint, Settings2, Edit3, CheckSquare, Plus, Loader2 } from 'lucide-react';
...
const RoleManagementModal: React.FC<RoleManagementModalProps> = ({ user, onClose, onUpdate, language, isManagement }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Verification: The success state is only reached if the promise resolves
      setShowSuccess(true);
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-dark/95 backdrop-blur-md animate-in fade-in duration-300 text-left">
       <div className="bg-surface border border-white/10 rounded-[3rem] p-10 w-full max-w-xl shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="overflow-y-auto pr-4 custom-scrollbar space-y-8">
            <div className="bg-dark/40 rounded-3xl p-6 border border-white/5">
                ...
                <div className="space-y-4">
                    ...
                    <div className="p-4 bg-white/5 rounded-2xl">
                       <p className="text-[8px] font-black uppercase text-slate-500 mb-1">Registered Email</p>
            ...
            <div>
               <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-4 italic">Authority Table</h3>
                    ...
                    return (
                      <button 
                        ...
                        className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${
                          isActive ? 'bg-brand/10 border-brand/30 text-white' : 'bg-dark/20 border-white/5 text-slate-500'
          ...
          <div className="pt-8 mt-8 border-t border-white/5">
             <button 
                ...
                className={`w-full py-5 rounded-2xl font-black uppercase tracking-widest text-[11px] transition-all shadow-xl flex items-center justify-center gap-2 ${
                    showSuccess ? 'bg-green-500 text-white' : 'bg-brand text-dark hover:bg-white shadow-brand/10'
                } disabled:opacity-50`}
             >
                {isSaving ? <Loader2 className="animate-spin" size={18} /> : 
                 showSuccess ? <><CheckSquare size={18} /> Verified & Saved</> : 
                 <><ShieldCheck size={18} /> Confirm Identity Update</>}
             </button>
          </div>
       </div>
    </div>
