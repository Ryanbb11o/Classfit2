...
import { Calendar, Star, LogOut, Dumbbell, Loader2, X, MapPinned, Settings2, Trash2, AlertCircle, Sparkles } from 'lucide-react';
...
const ReviewModal: React.FC<{ 
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-dark/95 backdrop-blur-md">
       <div className="bg-surface rounded-[2.5rem] border border-white/10 w-full max-w-md p-10 text-center relative shadow-2xl">
          ...
          <div className="w-16 h-16 bg-brand text-dark rounded-[1.5rem] flex items-center justify-center mx-auto mb-6"><Star size={28} className="fill-dark" /></div>
    ...
    <div className="max-w-5xl mx-auto px-4 py-24 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-12 bg-surface p-8 rounded-[2.5rem] border border-white/5 shadow-2xl relative overflow-hidden group">
         <div className="flex items-center gap-6 relative z-10">
            <div className="w-24 h-24 rounded-full p-1 border-2 border-brand/50 bg-dark"><img src={currentUser.image || DEFAULT_PROFILE_IMAGE} className="w-full h-full object-cover rounded-full" /></div>
            <div className="text-center md:text-left">
                ...
                <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                   ...
                   {isManagement && <button onClick={() => setShowEditModal(true)} className="bg-white/5 hover:bg-brand hover:text-dark text-white text-[9px] font-black uppercase tracking-[0.2em] px-3 py-1 rounded-full border border-white/10 transition-all flex items-center gap-1.5"><Settings2 size={10} /> Edit Identity</button>}
                </div>
            </div>
         </div>
                    ...
                    <div key={booking.id} className="bg-surface/50 backdrop-blur-md border border-white/5 rounded-[3rem] p-8 hover:border-brand/40 transition-all group overflow-hidden">
                        <div className="flex flex-col lg:flex-row gap-8 lg:items-center">
                            <div className="flex items-center gap-6 shrink-0">
                                <div className="w-20 h-20 rounded-2xl overflow-hidden shadow-xl bg-dark"><img src={getTrainerImage(booking.trainerId)} className="w-full h-full object-cover grayscale opacity-80 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-700" /></div>
                            ...
                            <div className="shrink-0 flex flex-col items-center justify-center p-6 bg-dark/40 rounded-[2rem] border border-white/10 min-w-[160px]">
                                ...
                                <div className={`mt-2 px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${booking.status === 'confirmed' ? 'bg-green-500/10 text-green-500' : 'bg-brand/10 text-brand'}`}>{booking.status.replace('_', ' ')}</div>
                            </div>
                        </div>
                        <div className="mt-6 pt-6 border-t border-white/5 flex flex-wrap items-center justify-between gap-4">
                            <div className="flex gap-2">
                                {booking.status === 'confirmed' && <button onClick={() => setBookingToReview(booking)} className="flex items-center gap-2 px-6 py-3 bg-brand text-dark rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all shadow-lg"><Star size={14} /> Finish & Review</button>}
