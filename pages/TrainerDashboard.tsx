            ...
            >
                ...
                <div className="flex flex-col gap-1.5 mt-2">
                   {hasPending && (
                      <div className="flex items-center gap-1.5 bg-yellow-500/10 px-2 py-0.5 rounded-md border border-yellow-500/20">
                         ...
                         <span className="text-[11px] font-black text-yellow-500 uppercase">Pending</span>
                      </div>
                   )}
                   {hasConfirmed && (
                      <div className="flex items-center gap-1.5 bg-green-500/10 px-2 py-0.5 rounded-md border border-green-500/20">
                         ...
                         <span className="text-[11px] font-black text-green-500 uppercase">Confirmed</span>
                      </div>
                   )}
                </div>
                ...
                <div className="mt-auto">
                  {dayBookings.length > 0 && (
                     <div className="text-[11px] font-black uppercase tracking-widest text-white italic bg-white/5 px-2.5 py-1 rounded-xl border border-white/10 inline-flex items-center gap-2">
    ...
    <div className="max-w-7xl mx-auto px-4 py-24 animate-in fade-in duration-500 text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <div className="p-8 bg-surface border border-white/5 rounded-[2.5rem] relative overflow-hidden">
             ...
             <p className="text-[11px] font-black uppercase tracking-widest text-slate-500 mb-4 italic">Confirmations Needed</p>
          ...
          <div className="p-8 bg-surface border border-white/5 rounded-[2.5rem] relative overflow-hidden">
             ...
             <p className="text-[11px] font-black uppercase tracking-widest text-slate-500 mb-4 italic">Active Sessions</p>
          ...
          <div className="p-8 bg-brand text-dark rounded-[2.5rem] shadow-xl relative overflow-hidden">
             ...
             <p className="text-[11px] font-black uppercase tracking-widest opacity-60 mb-4 italic">Completed Volume</p>
      ...
      <div className="bg-surface rounded-[3rem] border border-white/10 overflow-hidden min-h-[600px] shadow-2xl">
         <div className="p-6 border-b border-white/10 flex flex-col md:flex-row items-center justify-between gap-6 bg-dark/20">
            <div className="flex items-center gap-4 bg-dark/40 p-1.5 rounded-2xl border border-white/5">
               <button onClick={() => setActiveTab('calendar')} className={`px-6 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all ${activeTab === 'calendar' ? 'bg-brand text-dark shadow-lg' : 'text-slate-500 hover:text-white'}`}>Calendar</button>
               <button onClick={() => setActiveTab('requests')} className={`px-6 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all ${activeTab === 'requests' ? 'bg-brand text-dark shadow-lg' : 'text-slate-500 hover:text-white'}`}>Requests {pendingRequests.length > 0 && `(${pendingRequests.length})`}</button>
               <button onClick={() => setActiveTab('reviews')} className={`px-6 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all ${activeTab === 'reviews' ? 'bg-brand text-dark shadow-lg' : 'text-slate-500 hover:text-white'}`}>Reviews</button>
            </div>
                  ...
                  <div className="p-10 space-y-4 animate-in fade-in duration-500">
                     {confirmedSessions.length === 0 ? <p className="text-center py-20 text-slate-500 font-bold italic">No sessions scheduled.</p> : confirmedSessions.sort((a,b) => a.date.localeCompare(b.date)).map(b => (
                        <div key={b.id} className="p-6 bg-dark/20 border border-white/5 rounded-[2rem] flex items-center justify-between">
                           <div>
                              <p className="text-[11px] font-black uppercase text-brand tracking-widest mb-1 italic">{b.date} • {calculateTimeRange(b.time, b.duration)}</p>
                           ...
                           <div className="flex items-center gap-3">
                              ...
                              <button onClick={() => handleAction(b.id, 'trainer_completed')} className="px-5 py-2 bg-brand text-dark text-[11px] font-black uppercase tracking-widest rounded-xl hover:bg-white transition-all shadow-md shadow-brand/10">Mark Done</button>
                           </div>
                        </div>
                     ))}
                  </div>
                        ...
                        {pendingRequests.length === 0 ? <tr><td colSpan={3} className="px-8 py-20 text-center text-slate-500 font-bold italic">All requests processed. Ready for training.</td></tr> : pendingRequests.map(b => (
                           <tr key={b.id} className="hover:bg-white/5 transition-colors">
                              <td className="px-8 py-6">
                                 ...
                                 <p className="text-[11px] text-slate-500 font-black tracking-widest">{b.date} • {calculateTimeRange(b.time, b.duration)}</p>
                              </td>
                              <td className="px-8 py-6 text-center"><span className="px-3 py-1 bg-brand/10 text-brand rounded-lg text-[11px] font-black border border-brand/20 italic">{b.checkInCode}</span></td>
               ...
               <div className="p-12 grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in">
                  {getTrainerReviews(currentUser.id, language).map((r, i) => (
                     <div key={i} className="p-8 bg-dark/20 border border-white/5 rounded-[2.5rem] relative">
                        ...
                        <span className="absolute bottom-6 right-8 text-[11px] font-black uppercase tracking-widest text-slate-700 italic">{r.time}</span>
                     </div>
                  ))}
               </div>
            )}
         ...
         <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-dark/95 backdrop-blur-xl animate-in fade-in duration-300 text-left">
            <div className="bg-surface border border-white/10 rounded-[3rem] p-10 w-full max-w-2xl shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="mb-10">
                   ...
                   <div className="flex items-center gap-3">
                      <p className="text-[11px] font-black uppercase tracking-widest text-slate-500">Day Details & Management</p>
                      <button 
                        onClick={() => handleToggleBlockDate(selectedDay.toISOString().split('T')[0])}
                        className={`flex items-center gap-2 px-3 py-1 rounded-lg text-[11px] font-black uppercase tracking-widest transition-all ${
                ...
                <div className="space-y-4 overflow-y-auto custom-scrollbar flex-grow pr-4">
                   {myBookings.filter(b => b.date === selectedDay.toISOString().split('T')[0]).length === 0 ? (
                      <div className="p-12 text-center bg-white/5 border border-dashed border-white/10 rounded-2xl text-slate-500 text-[11px] font-black uppercase tracking-widest">No sessions found for this date.</div>
                   ) : (
                      myBookings.filter(b => b.date === selectedDay.toISOString().split('T')[0]).map(b => (
                         <div key={b.id} className="p-6 bg-dark/40 border border-white/5 rounded-2xl flex items-center justify-between group">
                            <div className="flex items-center gap-6">
                               <div className="text-center shrink-0"><p className="text-[11px] font-black text-slate-500 uppercase">TIME</p><p className="text-lg font-black text-white italic">{calculateTimeRange(b.time, b.duration)}</p></div>
                               <div>
                                  ...
                                  <div className="flex items-center gap-2">
                                     ...
                                     {b.status === 'confirmed' && (
                                       <a 
                                          ...
                                          target="_blank"
                                          rel="noopener noreferrer"
                                          className="text-[11px] text-brand uppercase font-black hover:underline flex items-center gap-1"
                                       >
                            ...
                            <div className="flex gap-2">
                               ...
                               {b.status === 'confirmed' && <button onClick={() => handleAction(b.id, 'trainer_completed')} className="px-4 py-2 bg-brand text-dark rounded-xl text-[11px] font-black uppercase hover:bg-white transition-all shadow-md">Mark Finished</button>}
                            </div>
                         </div>
                      ))
                   )}
