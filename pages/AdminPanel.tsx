...
const AdminPanel: React.FC = () => {
  ...
  // Tab State - IDs MUST match button IDs exactly
  ...
  const handleVerifyPayment = async (booking: Booking, method: 'cash' | 'card') => {
  ...
  const handleApproveApp = async (userId: string) => {
    ...
    const newRoles = currentRoles.filter(r => r !== 'trainer_pending' as any);
    if (!newRoles.includes('trainer')) newRoles.push('trainer');
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 py-16 animate-in fade-in duration-500 text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-12">
          ...
          <button 
            onClick={() => setActiveTab('finance')}
            className="p-8 bg-surface border border-white/5 rounded-[2rem] hover:border-brand/40 hover:shadow-[0_0_30px_rgba(197,217,45,0.3)] transition-all group active:scale-[0.98]"
          >
            ...
            {[
              ...
              { id: 'reviews', icon: Star, label: t.moderation, badge: pendingReviews.length }
                ...
                <button 
                  ...
                  className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center gap-2 transition-all ${activeTab === tab.id ? 'bg-brand text-dark' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                >
      ...
      <div className="space-y-8">
        ...
        {/* OVERVIEW TAB - RESTORED ACTIVE SESSIONS TABLE */}
        {activeTab === 'overview' && (
             ...
             <div className="bg-surface rounded-[2.5rem] border border-white/5 overflow-hidden shadow-xl">
                <div className="p-8 border-b border-white/5 bg-dark/20 flex items-center justify-between">
                   <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 flex items-center gap-3 italic">
                      <Target size={16} className="text-brand" /> Today's Live Operation Feed
                   </h3>
                   <span className="text-[10px] font-black text-brand bg-brand/10 px-3 py-1 rounded-lg uppercase italic border border-brand/20">LIVE PROTOCOL ACTIVE</span>
                </div>
                   ...
                   <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-white/5 text-[10px] italic">
                         {safeBookings.filter(b => b.date === todayStr).length === 0 ? (
                            <tr><td colSpan={5} className="p-10 text-center text-slate-600 font-black uppercase tracking-widest italic">No active deployments for today.</td></tr>
                            ...
                            <tr key={b.id} className="hover:bg-white/5 transition-colors">
                               ...
                               <td className="p-5">
                                  <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest ${b.status === 'confirmed' ? 'bg-green-500/10 text-green-500' : 'bg-yellow-500/10 text-yellow-500'}`}>{b.status}</span>
                               </td>
           ...
           <div className="bg-surface rounded-[2.5rem] border border-white/5 overflow-hidden shadow-2xl animate-in slide-in-from-bottom-2">
              <div className="p-6 border-b border-white/5 flex items-center justify-between bg-dark/20">
                 <h3 className="text-[11px] font-black uppercase tracking-widest text-white italic flex items-center gap-3">
                    <DollarSign size={18} className="text-brand"/> Settlement Protocol
                 </h3>
                 <span className="bg-red-500 text-white text-[9px] font-black uppercase px-3 py-1 rounded-full">{awaitingPaymentList.length} Awaiting</span>
              </div>
              <div className="max-h-[600px] overflow-y-auto custom-scrollbar">
                <table className="w-full text-left">
                   <thead className="bg-dark/10 text-[9px] font-black uppercase text-slate-500 sticky top-0 z-10 backdrop-blur-sm">
                      <tr><th className="p-4">Deployment</th><th className="p-4">Subject</th><th className="p-4">Officer</th><th className="p-4">Value</th><th className="p-4">Action</th></tr>
                   </thead>
                   <tbody className="divide-y divide-white/5 text-[10px] italic">
                      {safeBookings.filter(b => b.status === 'trainer_completed' || b.status === 'completed').map(b => (
                            ...
                            <td className="p-4">
                               {b.status === 'trainer_completed' ? (
                                  <div className="flex gap-2">
                                     <button onClick={() => handleVerifyPayment(b, 'cash')} className="px-3 py-1.5 bg-brand text-dark rounded-lg text-[8px] font-black uppercase">CASH</button>
                                     <button onClick={() => handleVerifyPayment(b, 'card')} className="px-3 py-1.5 bg-white text-dark rounded-lg text-[8px] font-black uppercase">CARD</button>
                                  </div>
                 ...
                 <tbody className="divide-y divide-white/5 text-[10px] italic">
                    {safeBookings.map(b => (
                       <tr key={b.id} className="hover:bg-white/5">
                          ...
                          <td className="p-5">
                             <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest ${b.status === 'completed' ? 'bg-brand text-dark' : 'bg-white/5 text-slate-500'}`}>{b.status}</span>
                          </td>
                          <td className="p-5">
                             <button onClick={() => confirmAction({ title: 'Delete Entry?', message: 'Irreversible deletion from mainframe.', onConfirm: () => deleteBooking(b.id) })} className="p-2 text-slate-500 hover:text-red-500 transition-colors"><Trash2 size={14}/></button>
                          </td>
                       </tr>
                    ))}
                 </tbody>
              ...
              {pendingApps.length === 0 ? <p className="text-center col-span-full py-20 text-slate-500 uppercase font-black italic tracking-widest">No pending recruitment requests found.</p> : pendingApps.map(u => (
                 <div key={u.id} className="p-8 bg-surface rounded-[2.5rem] border border-white/10 shadow-xl relative overflow-hidden italic">
                    ...
                    <div className="flex items-center gap-4 mb-8">
                       ...
                       <div>
                          <h4 className="text-xl font-black uppercase text-white tracking-tighter">{u.name}</h4>
                          <p className="text-[10px] font-bold text-yellow-500 uppercase tracking-widest">Candidate Coach</p>
                       </div>
                    </div>
                         ...
                         <tr key={u.id} className="hover:bg-white/5 group">
                            ...
                            <td className="p-5">
                               <div className="flex flex-wrap gap-1">
                                  {(u.roles || []).map(r => <span key={r} className={`px-1.5 py-0.5 bg-white/5 rounded text-[7px] font-black uppercase tracking-widest ${r.includes('admin') ? 'text-red-500' : r.includes('trainer') ? 'text-brand' : 'text-slate-500'}`}>{r.replace('_', ' ')}</span>)}
                               </div>
                            </td>
                            <td className="p-5 text-slate-500 font-bold">{(u.joinedDate || '').split('T')[0]}</td>
                            <td className="p-5">
                               <div className="flex justify-end gap-2">
                                  <button onClick={() => setUserForRoles(u)} className="p-2 bg-white/5 rounded-lg text-brand hover:bg-brand hover:text-dark transition-all shadow-md" title="Modify Authority"><Settings2 size={14}/></button>
                                  <button onClick={() => confirmAction({ title: 'Delete Subject?', message: 'Irreversible termination of data.', onConfirm: () => deleteUser(u.id) })} className="p-2 text-slate-600 hover:text-red-500 transition-colors"><Trash2 size={14}/></button>
                               </div>
                            </td>
                         </tr>
                      ))}
        ...
        {activeTab === 'reviews' && (
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in duration-500">
              {pendingReviews.length === 0 ? <p className="text-center col-span-2 py-20 text-slate-500 uppercase font-black italic tracking-widest">No feedback awaiting moderation at this stage.</p> : pendingReviews.map(r => (
                 <div key={r.id} className="p-8 bg-surface rounded-[2.5rem] border border-white/10 shadow-xl relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-center mb-6">
                       <div>
                          ...
                          <p className="text-[8px] text-slate-500 font-bold uppercase tracking-widest italic">{cleanName(safeUsers.find(u => u.id === r.trainerId)?.name) || 'Personnel'}</p>
                       </div>
                       <div className="flex text-brand">
                          {[...Array(Number(r.rating || 5))].map((_, i) => <Star key={i} size={8} fill="currentColor"/>)}
                       </div>
                    </div>
                    <div className="bg-dark/40 p-5 rounded-2xl border border-white/5 mb-8 shadow-inner">
                       <p className="text-xs text-slate-200 italic font-medium leading-relaxed whitespace-pre-wrap break-words">
                          "{r.text || "No descriptive content logged."}"
                       </p>
                    </div>
                    <div className="flex gap-2">
                       <button onClick={() => updateReview(r.id, { isPublished: true })} className="flex-1 py-4 bg-brand text-dark rounded-xl text-[9px] font-black uppercase shadow-lg hover:bg-white transition-all italic"><ThumbsUp size={12} className="inline mr-2"/> Approve</button>
                       <button onClick={() => deleteReview(r.id)} className="flex-1 py-4 bg-white/5 text-red-500 rounded-xl text-[9px] font-black uppercase border border-red-500/10 hover:bg-red-500 hover:text-white transition-all italic">Delete</button>
                    </div>
                 </div>
              ))}
           </div>
